...
*[Lab 1, 2 & 3]: General introduction to CDW* to get ourselves oriented for the workshop.
____
** {blank}
+
...
|*CDP Workload User (${user_id})* |<Same as Workshop Login Username>
|*CDP Workload Password* |<Setup password as per Step 1>
...
[IMPORTANT]
Enter your assigned workshop username in place of **${user_id}** in the textbox below the query editor.
Enter the provided S3 bucket name in place of **${bucket_name}** in the textbox below the query editor.
...
) ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n' STORED AS TEXTFILE LOCATION 's3a://${bucket_name}/cdw/airlines-raw/airlines-csv/flights' tblproperties("skip.header.line.count" = "1");
----
...
) ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n' STORED AS TEXTFILE LOCATION 's3a://${bucket_name}/cdw/airlines-raw/airlines-csv/planes' tblproperties("skip.header.line.count" = "1");
...
CREATE EXTERNAL TABLE ${user_id}_airlines_raw.airports_csv(
  ...
  lon DOUBLE
) ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n' STORED AS TEXTFILE LOCATION 's3a://${bucket_name}/cdw/airlines-raw/airlines-csv/airports' tblproperties("skip.header.line.count" = "1");
----
...
*Location:* s3a://<bucket_name>/data/warehouse/tablespace/external/hive/immiapac00_airlines.db/planes
...
As a final step here let’s run the same analytic query we ran against the Raw layer now in our Data Lakehouse DW, to see what happens with performance. From the cloudera console select the IMPALA virtual warehouse assigned to you, and open the HUE in the new tab.
...
Now run the following query again, inside the Impala editor.
...
+
____
Filter for Assets we created - below the Data Lakes on the right of the Search bar Filter, select TYPE to be Hive Table and Database name as <username>_airlines. The below part of the screen will update to reflect this selection.
...
+
____
You should now see the tables and materialized views that have been created in the <user_id>_airlines database. Now, select TYPE to be Iceberg Table and click on flights in the Name column to view more details on the table.

++++
<p align="center">
  <img width="600" height="400" src="cdw-media/media/image75.png">
</p>
++++
____
