...
The overall time for provisioning is nearly ~1.5 to ~3 hours (Total Time = Time Reqd for DataLake Provisioning + Time Reqd for CDE and/or CDW Provisioning). 

---

[NOTE]
If the execution of docker container for provisioning fails (due to timeout or any kind of intermittent/API issues at CDP/AWS side), you can use the same command mentioned above to refresh the state and resume the exceution.
...
[.shell]
----
rm -rf /userconfig/.accn-wrkshp
rm -rf /userconfig/accn-wrkshp.txt
----
---
